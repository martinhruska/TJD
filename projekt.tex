...
A~choice of abstract domain is not completely unrestricted.
The framework of abstract interpretation requires to chose a domain
overapproximation all possible behaviors of the original program.
...
Finally an algorithm for computing semantics (or its overapproximation) of program
...
Another necessary property of abstract interpretation leading to soundness is
...
modeling semantics of an instruction $I$ in concrete program.
...
used in computation of all possible behaviors of for a given program point.
...
Concrete domain is $2^\mathbb{\intg}$, so each variable
